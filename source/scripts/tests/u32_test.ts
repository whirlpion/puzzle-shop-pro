async function u32_test() {
    await test.begin("u32");
    try {
        let u: u32 = u32.fromNumber(0);
        // let u2: u32 = u32.fromNumber(0);
        let threw = false;

        await test.println("can be constructed from bytes");
        u = u32.fromBytes(0x00, 0x01, 0x02, 0x03);
        throwIfNotEqual(u.value, 0x03020100);

        await test.println("can be constructed from an positive integer less than 2^32");
        u = u32.fromNumber(0x00010203);
        throwIfNotEqual(u.value, 0x00010203);

        await test.println("should throw if constructed from a float");
        threw = false;
        try {
        ...
        await test.println("should throw if constructed from negative integer");
        threw = false;
        try {
            u = u32.fromNumber(-1);
        } catch(ex) {
        ...
        await test.println("should throw if constructed from integer equal to 2^32");
        threw = false;
        try {
        ...
        await test.println("should throw if constructed from integer greater than 2^32");
        threw = false;
        try {
        ...
        await test.println("shuld throw if rotated by too many bits");
        threw = false;
        try {
        ...
        await test.println("should throw if attempting to rotate by negative bits");
        threw = false;
        try {
        ...
        throwIfFalse(threw);
        await test.println("should be unchanged if xor with 0");
        u = u32.xor(u32.fromNumber(0x00010203), u32.fromNumber(0x00000000));
        throwIfNotEqual(u.value, 0x00010203);

        await test.println("can be xor'd with another u32");
        u = u32.xor(u32.fromNumber(0x00010203), u32.fromNumber(0x04050607));
        throwIfNotEqual(u.value, 0x4040404);

        await test.println("can be shifted right by a given number of bits");
        ...
        await test.println("should throw if attempting to shift by too many bits");
        threw = false;
        try {
        ...
        await test.println("should throw if attempting to shift by negative bits");
        threw = false;
        try {
        ...
        await test.println("should be printable as string");
