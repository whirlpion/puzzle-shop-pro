"use strict";
class Grid8x8HorizontalConstraint extends IConstraint {
    constructor(sceneManager, cell) {
        ...
        (() => {
            ...
            {
                let rect = sceneManager.createElement("rect", SVGRectElement);
                rect.setAttributes(["width", `${4 * CELL_SIZE}`], ["height", `${8 * CELL_SIZE}`], ["stroke", Colour.Black.toString()], ["stroke-width", `${CELL_BORDER_SIZE}`], ["fill", "none"], ["x", `${2 * CELL_SIZE}`], ["y", "0"]);
                group.appendChild(rect);
            }
            // horizontal rects
            {
                let rect = sceneManager.createElement("rect", SVGRectElement);
                rect.setAttributes(["width", `${8 * CELL_SIZE}`], ["height", `${4 * CELL_SIZE}`], ["stroke", Colour.Black.toString()], ["stroke-width", `${REGION_BORDER_SIZE}`], ["fill", "none"], ["x", "0"], ["y", `${2 * CELL_SIZE}`]);
                group.appendChild(rect);
            }
            {
                let rect = sceneManager.createElement("rect", SVGRectElement);
            ...
            return graphic;
        })(), `grid-8x8-horizontal-${Grid8x8HorizontalConstraint.counter++}`);
        ...
        for (let i = 0; i < 4; i++) {
            for (let j = 0; j < 2; j++) {
                this.regionConstraints.push(RegionConstraint.RectangleRegion(new Cell(row + 2 * i, column + 4 * j), 4, 2));
            }
        }
    }
...
Grid8x8HorizontalConstraint.counter = 0;
